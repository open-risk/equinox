function timeseries_graph_ordinal(title, dates, values, metrics) {
    ...
    let ylabel_y = -20;
    let ylabel_x = graph_object.width / 2;
    ...
    svg_graph.append("line")
        .attr({
            x1: x(parseTime.parse(dates[0])), y1: y(2),
            x2: x(parseTime.parse(dates[data.length - 1])), y2: y(2)
        })
    ...
    svg_graph.append("line")
        .attr({
            x1: x(parseTime.parse(dates[0])), y1: y(1),
            x2: x(parseTime.parse(dates[data.length - 1])), y2: y(1)
        })
        .attr("stroke-dasharray", ("3, 3"))
        .attr("stroke-width", 1)
        .style("fill", "#5b4421")
    ...
    svg_graph.append("line")
        .attr({
            x1: x(parseTime.parse(dates[0])), y1: y(3),
            x2: x(parseTime.parse(dates[data.length - 1])), y2: y(3)
        })
        .attr("stroke-dasharray", ("3, 3"))
        .attr("stroke-width", 1)
        .style("fill", "#5b4421")
